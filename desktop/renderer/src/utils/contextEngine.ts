...
import { SearchResult, searchProject, searchInContent as searchInContentEngine, SearchOptions, DEFAULT_SEARCH_OPTIONS } from './searchEngine';
...
// Extract keywords from question (Arabic + Latin aware)
export function extractKeywords(question: string): string[] {
  // Arabic stop words (common function words that add no search value)
  const arabicStopWords = new Set([
    'في', 'من', 'على', 'عن', 'إلى', 'الى', 'مع', 'هذا', 'هذه', 'ذلك',
    'التي', 'الذي', 'الذين', 'ما', 'لا', 'أن', 'ان', 'إن', 'اذا', 'إذا',
    'كل', 'بعض', 'غير', 'بين', 'أو', 'او', 'ثم', 'قد', 'كان', 'كانت',
    'يكون', 'هو', 'هي', 'هم', 'أين', 'اين', 'كيف', 'لماذا', 'متى',
    'أظهر', 'اعرض', 'أرني', 'ارني', 'أعطني', 'اعطني', 'وجد', 'ابحث',
  ]);
  // English stop words
  const stopWords = new Set([
    ...
    'find', 'show', 'me', 'get', 'list', 'give',
  ]);
  
  // \\p{L} matches Arabic letters too (\\w does not) — the previous regex
  // silently discarded every Arabic word, leaving zero keywords.
  const words = question
    .toLowerCase()
    .replace(/[^\p{L}\p{N}\s_]/gu, ' ')
    .split(/\s+/)
    .filter(word => word.length > 1 && !stopWords.has(word) && !arabicStopWords.has(word));
...
export function extractContext(
  ...
  const searchResults = searchProject(index, keywords.join(' ') || query, { maxResults: maxFiles }, fileContents);
...
export function searchInContent(
  ...
  // Normalize the query to plain tokens (Arabic/Latin aware) — the raw
  // natural-language question would rarely appear verbatim in source code.
  const tokens = extractKeywords(query);
  if (tokens.length === 0) return [];
  const options: SearchOptions = { ...DEFAULT_SEARCH_OPTIONS, maxResults: 100 };
  const seen = new Set<string>();
  const out: { line: number; text: string; highlight: string }[] = [];
  for (const token of tokens) {
    for (const m of searchInContentEngine(content, token, options)) {
      const key = `${m.line}:${m.text}`;
      if (!seen.has(key)) {
        seen.add(key);
        out.push({ line: m.line || 0, text: m.text, highlight: token });
      }
    }
  }
  return out.sort((a, b) => a.line - b.line).slice(0, 50);
}
