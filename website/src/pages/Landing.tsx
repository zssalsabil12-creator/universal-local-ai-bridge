...
function Navbar({ onLaunch }: { onLaunch: () => void }) {
  const { t } = useLanguage();
  const downloadUrl = 'https://github.com/zssalsabil12-creator/universal-local-ai-bridge/releases/download/v3.10.10/ULAB-Setup-3.10.10-Windows-x64.exe';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            <LanguageSwitcher />
            <a href={downloadUrl} download="ULAB-Setup-3.10.10-Windows-x64.exe" className="hidden md:inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#111827]/80 border border-cyan-400/25 text-cyan-200 text-sm font-bold hover:border-cyan-300/50 hover:bg-cyan-400/10 transition-all">
        ...
        {mobileOpen && (
          ...
          >
            <div className="px-4 py-4 space-y-3">
              ...
              <a href={downloadUrl} download="ULAB-Setup-3.10.10-Windows-x64.exe" onClick={() => setMobileOpen(false)} className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-xl border border-cyan-400/25 bg-cyan-400/10 text-cyan-200 text-sm font-bold">
                <Download className="w-4 h-4" />
                {t('nav.download')}
              </a>
          ...
          <div className="relative p-12 sm:p-20 rounded-[3rem] bg-[#111118] border-2 border-purple-500/30 hover:border-purple-500/50 transition-all duration-500">
            <div className="text-center">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <motion.a
                  href={downloadUrl}
                  download="ULAB-Setup-3.10.10-Windows-x64.exe"
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
