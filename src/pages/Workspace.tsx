...
import {
  ...
  getFileIcon, shouldIgnore, writeFileContent, exportProjectToZip
...
import DirectAIPasteModal from '../components/DirectAIPasteModal';
import LargeProjectAssistantModal from '../components/LargeProjectAssistantModal';
import AdContainer from '../components/AdContainer';
import LegalModal from '../components/LegalModal';
import LocalAgentStatusModal from '../components/LocalAgentStatusModal';
...
import { localAgent, useAgentConnection, AuditLogEntry } from '../utils/localAgent';
...
export default function Workspace({ onBack }: { onBack: () => void }) {
  ...
  const [showLargeProjectAssistant, setShowLargeProjectAssistant] = useState(false);
  ...
  const startAgentTask = (action: { type: 'ulp_command' | 'file_modification' | 'info'; description: string; metadata?: any }) => {
    const taskType = action.type === 'file_modification' ? 'file_write' :
      action.metadata?.taskType || 'ai_query';
    return addTask({
      type: taskType,
      status: 'running',
      title: action.description,
      description: action.description,
      metadata: { source: 'agent-command-center', ...action.metadata },
    });
  };

  const completeAgentTask = (taskId: string, success: boolean, durationMs: number, metadata?: any) => {
    updateTask(taskId, {
      status: success ? 'completed' : 'failed',
      completedAt: Date.now(),
      duration: durationMs,
      metadata: { source: 'agent-command-center', ...metadata },
    });
    addLog(success ? 'Agent مهمة مكتملة' : 'Agent مهمة فاشلة', taskId);
  };
  ...
  const handleOpenFolder = async () => {
    try {
      ...
      if (e.name !== 'AbortError') {
        ...
        notifications.error('فشل العملية', e.message || 'فشل فتح المجلد');
      }
    }
  };
  ...
  return (
    <div className="ulab-workspace h-screen flex flex-col overflow-hidden">
      {/* Header */}
      <header className="ulab-topbar h-14 flex items-center px-4 gap-3 flex-shrink-0">
        ...
        <div className="ulab-topbar-actions flex items-center gap-2 mr-auto">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold bg-[#161622] hover:bg-[#202030] text-[#94a3b8] hover:text-[#e2e8f0] border border-[#2a2a3a] transition-all"
            title="فحص حالة اتصال ULAB Local Agent وSandbox"
          >
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="hidden sm:inline">Local Agent V1</span>
          </button>

          {/* Apply AI Code Button */}
          <button
            onClick={() => setShowAIPasteModal(true)}
            className="flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold bg-gradient-to-r from-cyan-500 to-indigo-500 text-white hover:shadow-md hover:shadow-cyan-500/20 transition-all"
            title="الصق ردود DeepSeek / ChatGPT لتطبيق التغييرات تلقائياً"
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span>تطبيق رد AI</span>
          </button>

          {/* Large Project Slicer Button */}
          <button
            onClick={() => setShowLargeProjectAssistant(true)}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-[#1e1e2d] hover:bg-[#28283d] text-purple-300 border border-purple-500/30 transition-colors"
            title="مساعد المشاريع الضخمة وضغط السياق لـ DeepSeek"
          >
            <Layers className="w-3.5 h-3.5 text-purple-400" />
            <span className="hidden md:inline">مساعد المشاريع الكبيرة</span>
          </button>
          ...
          <div className="flex-1 overflow-hidden">
            ...
            {activePanel === 'agent' && (
              <AgentModePanel
                ...
                onTaskStart={startAgentTask}
                onTaskComplete={completeAgentTask}
                onRefreshAudit={refreshAudit}
                onExecuteCommand={async (cmd, params) => {
                  if (permissionMode === 'readonly' && (cmd === 'terminal.execute' || cmd === 'terminal.run' || cmd === 'files.write' || cmd === 'files.delete' || cmd === 'workspace.applyChange')) {
                    notifications.warning('العملية محظورة', 'وضع القراءة فقط يسمح بعمليات الاستعلام دون التعديل أو التنفيذ.');
                    return { success: false, error: { message: 'READONLY_MODE' } };
                  }
                  const result = await localAgent.sendRequest(cmd, params || {});
                  addLog('Agent Execute', `${cmd} ${JSON.stringify(params || {})}`);
                  if (!result.success) {
                    notifications.error('فشل تنفيذ الأمر', result.error?.message || 'تعذر تنفيذ الطلب');
                  }
                  return result;
                }}
                onApplyModification={applyFileModification}
              />
            )}
      ...
      {/* Direct AI Code Paste Modal (for DeepSeek, ChatGPT, Claude) */}
      <DirectAIPasteModal
        isOpen={showAIPasteModal}
        onClose={() => setShowAIPasteModal(false)}
        fileContents={fileContents}
        onApplyToFile={applyFileModification}
        onBatchApply={async (files) => {
          for (const f of files) {
            await applyFileModification(f.path, f.content);
          }
        }}
        projectName={projectIndex?.rootName || 'Project'}
        hasLocalDirectory={!!dirHandle}
      />

      {/* Large Project Context Slicer Modal */}
      <LargeProjectAssistantModal
        isOpen={showLargeProjectAssistant}
        onClose={() => setShowLargeProjectAssistant(false)}
        projectIndex={projectIndex}
        fileContents={fileContents}
        onReadFile={handleReadFile}
      />

      {/* Legal & Privacy Modal (Google AdSense Compliance) */}
      <LegalModal
